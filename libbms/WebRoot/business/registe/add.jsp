...
<html>
	...
	<div id="contentbar">
		...
		<tr><td><span class="mustfill">*</span>委托单位：</td><td>
			  ...
			  <input type="hidden" id="enid" name="sampleRegiste.entrustcompanyid" />
			  <input type="text" id="adminAreaCode" onkeyup="inputchange()" name="sampleRegiste.entrustcompany" value="" class="longtext"/><span class="textdesc"> 输入查询</span>
		...
		<tr id="tr_input_user"><td><span class="mustfill">*</span>送样人：</td><td>
			    ...
			    <input type="hidden" id="inputuser" name="sampleRegiste.senduserid" />
			    <input type="text" id="user" onkeyup="inputusername()" name="sampleRegiste.senduser" value="" class="longtext"/><span class="textdesc"> 输入查询</span>
		...
		<!-- 曾智琴 -->
		<tr><td><span class="mustfill">*</span>对接人：</td><td><s:select name="sampleRegiste.acceptpeople" id="selectExamineuser" headerKey="" headerValue="--请选择--" list="map" /></td></tr>
		<tr><td><span class="mustfill">*</span>样品来源：</td><td><s:textfield name="sampleRegiste.resource"  id="selectResource" data-validation="length" data-validation-length="0-50" data-validation-error-msg="字数范围在(0-50)" cssClass="shorttext"></s:textfield></td></tr>
		<tr><td><span class="mustfill">*</span>委托分类：</td><td><s:select name="sampleRegiste.samplecategoryid"  id="selectSampleCategory" headerKey="" headerValue="--请选择--" list="samplecategorymap"/></td></tr>
	 <!--<tr><td>检验标准：</td><td><s:select name="sampleRegiste.checkstandardid"  id="selectSampleCategory" headerKey="" headerValue="--请选择--" list="checkstandardmap"/></td></tr>  -->
		<%--  <tr><td><span class="mustfill">*</span>检验标准：</td><td>
			  <s:select  list="checkstandardmap"  id="hiddensel2" style="display:none"></s:select>	
			  <input type="hidden" id="enid2" name="sampleRegiste.checkstandardid" />		  
			  <input type="text" id="adminAreaCode2" onkeyup="inputchange2()"  value="" class="longtext"/><span class="textdesc"> 输入查询</span>
		...
		<tr><td></td><td id="itemlist"><div><input type="checkbox" id="addcheckbox"/>  项目：<s:textfield id="firstInputItem" name="inputCheckItems" data-validation="length" data-validation-length="1-500" data-validation-error-msg="字数范围在(1-500)"  cssClass="longtext"></s:textfield>  室主任：<s:select name="inputExamineusers" id="inputExamineusers" headerKey="" headerValue="--请选择--" list="map"/> | <a href="javascript:void(0)" onclick="removeItem(this)">移除</a></div></td></tr>
		<tr><td></td><td><input type="checkbox" name="selectcontrol" id="selectall" onclick="allselected(this)">全选</input><input type="checkbox" name="selectcontrol" id="selectpart" onclick="partselected()">反选</input>&nbsp;&nbsp;&nbsp;&nbsp;  室主任：<s:select name="selectExamineuser" id="selectExamineuser" headerKey="" headerValue="--请选择--" list="map" onchange="updatecheck()"/><span class="mustfill">* 选择此室主任后，上面选中项自动变更</span></td></tr>
		...
		<tr><td><span class="mustfill">*</span>付款方式：</td><td><s:radio list="#{'2':'直接支付','0':'待定'}" name="sampleRegiste.payway" value="2" checked="checked" onclick="showContent()" /></td></tr>
        <tr id="tr_paydes"><td><span class="mustfill">*</span>付款方式说明：</td><td><s:textfield id="paydes" name="sampleRegiste.paydes" value="待付款方式确定后支付" data-validation="length" data-validation-length="1-200" data-validation-error-msg="字数范围在(1-200)" cssClass="longtext"></s:textfield></td></tr>
...
<!--  		<tr id="tr_accordcontent"><td>依据内容：</td><td><s:textfield id="accordings" name="sampleRegiste.accordings"  data-validation="length" data-validation-length="0-200" data-validation-error-msg="字数范围在(0-200)" cssClass="longtext"></s:textfield></td></tr>-->
		<tr><td><span class="mustfill">*</span>是否出具报告：</td><td> <s:radio list="#{'0':'不出报告','1':'出具科研测试报告','2':'出具检测报告'}" name="sampleRegiste.reporttype" value="2" onclick="showTalent()"/></td></tr>
<!--		<tr id="tr_talent"><td><span class="mustfill">*</span>资质：</td><td><s:select name="sampleRegiste.talent"  list="qualificationmap" cssClass="shortext"></s:select></td></tr>-->
		<tr id="tr_talent"><td><span class="mustfill">*</span>资质：</td><td><s:checkboxlist list="#{'1':'CMA','2':'CNAS','3':'CMAF','4':'其他'}" name="sampleRegiste.talent" value="4" /></td></tr>
...
<!-- 		<tr><td>报告快递单号：</td><td><s:textfield name="sampleRegiste.reportno"  data-validation="length" data-validation-length="0-200" data-validation-error-msg="字数范围在(0-200)" cssClass="longtext"></s:textfield></td></tr> -->
		...
		<tr><td colspan="2" align="center"><s:submit id="ok" value="保 存" onclick="return check()"></s:submit><input value="返 回" type="button" onclick="history.back();" /></td></tr>
		</table>
		</s:form>
        </div>
		</div>
		...
		function selCheckItems(){
		   ...
		   $("#itemlist").append('<div  style="height:30px"><input type="checkbox" id="addselcheckbox'+selcount+'"/>  项目：<select id="selitem'+selcount+'" name="selectCheckItems" cssClass="longtext"/> 室主任：<select id="selexamieuser'+selcount+'" name="selectExamineusers"/> | <a href="javascript:void(0)" onclick="removeItem(this)">移除</a></div>');
        ...
        function allselected(o){
            if (o.checked==true){ 
	            var firstobj = document.getElementById("addcheckbox");
	            if (firstobj!=null){
        ...
        function partselected(){
            var firstobj = document.getElementById("addcheckbox");
            if (firstobj!=null){
                 firstobj.checked=!firstobj.checked;
            }
            for(var i=0;i<count;i++){
                var obj = document.getElementById("addcheckbox"+i);
                if (obj!=null){
        ...
        function updatecheck(){
            ...
            if (firstobj!=null){
                 if (firstobj.checked){
                    var obj= document.getElementById("inputExamineusers");
	    ...
	    function check(){
	          ...
	          var objs = document.getElementsByName("sampleRegiste.checkstandardid");
	          for(var i=0;i<objs.length;i++){
                    if (objs[i].value==""){
                        var yq = document.getElementById("adminAreaCode2");
                        if (yq!=null){
                           alert("检验标准输入不正确，没有名称为："+yq.value+" 的标准，请检查！");
                           yq.focus();
                           return false;
                        }else{
                           alert("检验标准输入不正确，没有该项检验标准，请检查！");
                           return false;
                        }
                    }	          
	          }
