...
<html>
	...
	<div id="contentbar">
		<h1>修改结算</h1>
		<s:form action="updateIncome" namespace="/business">
		<table class="frmtable">
		<s:hidden id="incomeid" name="income.id"></s:hidden>
		<s:hidden id="savecount" value="%{regvalueList.size()}"></s:hidden>
		<tr><td><span class="mustfill">*</span>付款单位：</td><td>
<!--               <s:select  list="entrustcompanyList"  id="hiddensel" style="display:none"></s:select> -->
			  <input type="hidden" id="enid" name="income.entrustcompanyid" value="${income.entrustcompanyid }"/>
			  <input type="text" id="company" readonly="true" name="income.entrustcompany" value="${income.entrustcompanyStr }" class="hiddenlongtext"/>
		...
		<tr><td><span class="mustfill">*</span>联系人：</td><td>
                ...
                <s:textfield type="hidden" id="userid" name="income.userid"/>
			    <s:textfield type="text" id="user" onkeyup="inputuserchange()" name="income.username" class="longtext"/><span class="textdesc"> 输入查询</span>
		...
		<tr><td><span class="mustfill">*</span>付款日期：</td><td><s:textfield id="paydate" name="income.paydate" data-validation="date" data-validation-format="yyyy-mm-dd" data-validation-error-msg="不能为空" readonly="readonly" cssClass="shorttext">
		   <s:param name="value"><s:date name="income.paydate" format="yyyy-MM-dd"/></s:param>
		</s:textfield></td></tr>
				<s:if test='income.paytype==1'>
		   <tr><td>结算方式：</td><td><div><input type="checkbox" id="precal" name="income.precal" onclick="clearSyscal(this)" checked="true"/>预付款结算 | <a href="javascript:void(0)" onclick="addRegiste()">添加需结算登记单</a></div></td></tr>
		</s:if>
		<s:elseif test='income.paytype==2'>
        ...
        <s:iterator value="regvalueList" status="st">    
		     <tr><td></td><td id="calculate"><div id="modidiv<s:property value="#st.index"/>">样品登记： <s:select id="modisel%{#st.index}" name="registeid" list="regList" value="key" onchange="getBalance(modisel%{#st.index},moditxt%{#st.index})" />未付费用： <s:textfield id="moditxt%{#st.index}" value ="%{value}" cssClass="hiddenshorttext" readonly="true"/><a href="javascript:void(0)" onclick="removeItem(modidiv<s:property value="#st.index"/>)">移除</a></div></td></tr>
		</s:iterator>
		<tr><td>合计检测费：</td><td><s:textfield id="jcje" readonly="true" name="iname" cssClass="hiddenlongtext"></s:textfield>
		<tr id="tr_pay"><td><span class="mustfill">*</span>付款金额：</td><td><s:textfield id="nowcash" name="income.paymoney" data-validation="number" data-validation-allowing="float" data-validation-error-msg="只能输入数字" cssClass="shorttext"></s:textfield></td></tr>
		...
		<tr><td>发票类型：</td><td><s:radio id="optbill" list="#{'1':'增值税专用发票','2':'增值税普通发票','3':'未确定'}" name="income.taxtype"  /></td></tr>
		<tr><td><span class="mustfill">*</span>发票是否开具：</td><td><s:radio id="optbill" list="#{'1':'已开','0':'未开'}" name="income.billischeck"  onclick="billcheck()" /></td></tr>
		...
		function calneedmoney(){
          ...
          for(var i=0;i<parseInt(savecount);i++){
             var obj = document.getElementById("moditxt"+i);
             if(obj!=null&&obj!="undefined"){
                money = parseFloat(money)+parseFloat(obj.value);
             }
          }
		...
		function check(){
		      var objs = document.getElementsByName("income.registeid");
		      if ($("input:checkbox[name='income.caltype']:checked").val()!="on"&&count==0&&objs==null){
		          alert("请增加样品登记单 ！ ");
		          return false;
		      }
		      var money = 0;
	          ...
	          if($("select[name=registeid]").length>=2){
				    ...
				    var objs = document.getElementsByName("registeid");
				    var len = objs.length;
                    for(var i=0;i<len;i++){
                        var val = objs[i].options[objs[i].selectedIndex].value;
                        arrText[i] = val;
                    }
                   /*
				    var idx = 0;
